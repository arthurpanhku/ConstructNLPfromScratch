...
# Placeholder imports (assuming these exist in your project)
...
def dpo_loss(ref_probs: torch.Tensor, probs: torch.Tensor, beta: float = 0.1) -> torch.Tensor:
    ...
    Args:
        ref_probs: Log probabilities from the reference model (batch_size, seq_len)
        probs: Log probabilities from the policy model (batch_size, seq_len)
        beta: Temperature parameter controlling the strength of the preference (default: 0.1)

    Returns:
        torch.Tensor: Mean DPO loss across the batch
    """
    ...
    # Calculate the log-sigmoid preference loss
    # The policy should prefer chosen over rejected, adjusted by reference probs
    policy_log_ratio = chosen_probs - reject_probs
    ref_log_ratio = chosen_ref_probs - reject_ref_probs
